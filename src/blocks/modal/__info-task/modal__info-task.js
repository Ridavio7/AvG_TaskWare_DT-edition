...
let tasks_datebegt = document.getElementById("tasks_datebegin_time");
let tasks_dateacc  = document.getElementById("tasks_dateaccept");
let tasks_dateacct = document.getElementById("tasks_dateaccept_time");
let tasks_dateend  = document.getElementById("tasks_dateend");
let tasks_dateendt = document.getElementById("tasks_dateend_time");
let tasks_dtpredel = document.getElementById("tasks_dateendpredel");
let tasks_dtpredelt= document.getElementById("tasks_dateendpredel_time");
...
let tasks_dtstartt = document.getElementById("tasks_datestart_time");
...
const funcProcessGetTasksSteps = (result, respobj) => {
    ...
    if(respobj.answ.uin === '0'){
        ...
        tasks_dtstart.parentElement.parentElement.parentElement.classList.remove("modal__input-wrapper_display-none");
        ...
        tasks_dtstart.parentElement.parentElement.parentElement.classList.add("modal__input-wrapper_display-none");
...
(name, mission, prim, numb_i, dl, nameStatus, uinStatus, dtstart, dateaccept, datebegin, dateend, dtpredel, nameUser, uinUser, nameAreaprof, uinAreaprof, nameStart, uinStart, nameContent, uinContent, nameProd, uinProd, nameProc, uinProc, autoready, fproblem, fareaprof, fpart, count, countreal, uin, del, nameUsermove, datemove) => {
    ...
    tasks_dateacc.value   = dateaccept != undefined ? dateaccept.split("T")[0] : '';
    tasks_dateacct.value  = dateaccept != undefined ? dateaccept.split("T")[1] : '';
    tasks_datebeg.value   = datebegin  != undefined ? datebegin.split("T")[0] : '';
    tasks_datebegt.value  = datebegin  != undefined ? datebegin.split("T")[1] : '';
    tasks_dateend.value   = dateend    != undefined ? dateend.split("T")[0] : '';
    tasks_dateendt.value  = dateend    != undefined ? dateend.split("T")[1] : '';
    tasks_dtpredel.value  = dtpredel   != undefined ? dtpredel.split("T")[0] : '';
    tasks_dtpredelt.value = dtpredel   != undefined ? dtpredel.split("T")[1] : '';
    ...
    taskCountLastValue = tasks_count.value;
...
/* отслеживание count */
let taskCountChangeValue = false;
tasks_count.addEventListener('input', function() {
    const currentValue = tasks_count.value;

    if (currentValue !== taskCountLastValue) {
        taskCountLastValue = currentValue;
        taskCountChangeValue = true;
    }
})
...
tasks_save.onclick = () => {
    ...
    body.dateendpredel = (!tasks_dtpredel.value && !tasks_dtpredelt.value) ? "" :
                         (tasks_dtpredel.value && !tasks_dtpredelt.value) ? tasks_dtpredel.value :
                         (!tasks_dtpredel.value && tasks_dtpredelt.value) ? tasks_dtpredelt.value :
                         tasks_dtpredel.value + "T" + tasks_dtpredelt.value;
    body.datestart     = (!tasks_dtstart.value && !tasks_dtstartt.value) ? "" :
                         (tasks_dtstart.value && !tasks_dtstartt.value) ? tasks_dtstart.value :
                         (!tasks_dtstart.value && tasks_dtstartt.value) ? tasks_dtstartt.value :
                         tasks_dtstart.value + "T" + tasks_dtstartt.value;
    ...
    if(tasks_save.value == 0 && taskCountChangeValue === true){
        let result = confirm('Поменять количество во всех шагах задачи?');
        if(result){
            let body_2 = { "user":`${localStorage.getItem('srtf')}`, "meth":"update", "obj":"catTask", "uinTask":`${localStorage.getItem('uinTask')}`, "count":`${tasks_count.value}`};
            funcCommand(body_2, funcProcessOnlyInfo);

            taskCountChangeValue = false;
        }
    }
}
