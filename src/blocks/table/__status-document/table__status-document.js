...
customSortSelect("sort_statuses_statusdoc");
const dropdown = document.getElementById("sort_statuses_statusdoc");
const options  = dropdown.querySelectorAll('li');
options.forEach(option => {
    option.addEventListener('click', () => {
        switch (option.getAttribute('data-value')){
            case '1':
                let body1  =  {"user":`${localStorage.getItem('srtf')}`, "meth":"view", "obj":"statusdoc", "count":"5000", "sort":"name"};
                funcCommand(body1, funcProcessGetStatusDoc);
            break;
            case '2':
                let body2  =  {"user":`${localStorage.getItem('srtf')}`, "meth":"view", "obj":"statusdoc", "count":"5000", "asort":"name"};
                funcCommand(body2, funcProcessGetStatusDoc);
            break;
            case '3':
                let body3  =  {"user":`${localStorage.getItem('srtf')}`, "meth":"view", "obj":"statusdoc", "count":"5000", "sort":"uin"};
                funcCommand(body3, funcProcessGetStatusDoc);
            break;
            case '4':
                let body4  =  {"user":`${localStorage.getItem('srtf')}`, "meth":"view", "obj":"statusdoc", "count":"5000", "asort":"uin"};
                funcCommand(body4, funcProcessGetStatusDoc);
            break;
        }
    })
})