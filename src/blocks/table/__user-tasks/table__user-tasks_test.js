import {funcCommand, setStatus, responseProcessor, formatDate, funcProcessOnlyInfo, formatTime} from '../../../js/common/common.js.js';
...
export const funcGetUserTasksThreeColl = () => {
    let body  =  {"user":`${localStorage.getItem('srtf')}`, "meth":"view", "obj":"usersteps", "reg":"status2", "uinuser":`${localStorage.getItem('user_uin')}`, "count":"100", "sort":"datebegin"};
...
function buildStructureThreeColl(data) {
    ...
    for (const item of data) {
        for (const steps of item.steps) {
                ...
                <div class="user-tasks-card__dates-status">
                    <div class="user-tasks-card__dates">
                        <div class="user-tasks-card__date-item">
                            ...
                            <div class="user-tasks-card__date-value">${steps.status.uin >= 4 ? formatDate(steps.dateaccept).split(' ')[0] : formatDate(steps.datebegin).split(' ')[0]}</div>
                        </div>
                        <div class="user-tasks-card__date-item">
            ...
            steps.dateaccept, steps.dateend, steps.countprev, steps.mission, steps.prim, steps.uin, steps.fproblem, steps.status.uin, steps.countstep, steps.countreal, steps.fpart, steps.content.uin, steps.dl.dl_d, steps.dl.dl_h, steps.dl.dl_m));*/
            ...
            /*if(steps.status.uin >= 4){
                switch (steps.content.uin) {
                    case 2:
                        ...
                        break
                    case 3:
                        document.getElementById(`user_task_link_${steps.uin}`).firstElementChild.insertAdjacentHTML('beforeend', funcTaskContentMount(steps.uin));
...
const userTasksContentColl = (task_name, task_uin, product, uinProd, techproc, uinTechproc, count, tabcontent_id, name, admin, dateaccept, dateend, countprev, mission, prim, uin, fproblem, status, countstep, countreal, fpart, content, dl_d, dl_h, dl_m) => {
    return `
                ...
                <div class="user-tasks-content__detail-body">
                    <div class="user-tasks-content__detail-grid">
                        ...
                        <div class="user-tasks-content__info-item">
                            ...
                            <div class="user-tasks-content__info-value">${formatDate(dateaccept)}</div>
                        </div>
                        <div class="user-tasks-content__info-item">
                        ...
                        <div class="user-tasks-content__info-item">
                            <div class="user-tasks-content__info-label"></div>
                            <div class="user-tasks-content__info-value"></div>
                        </div>
                    </div>
